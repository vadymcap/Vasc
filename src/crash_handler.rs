...
pub fn hook() {
	panic::set_hook(Box::new(|panic_info| {
		...
		let mut report = String::from("> This crash report was automatically generated by vasc");
		report.push_str("\n\nDetails:\n--------\n");
		report.push_str("*Provide extra details about your problem or what happened here*");

		vasc_error!("{}", "vasc has crashed!".bold());

		report.push_str("\n\nMessage:\n----------\n");
		report.push_str(message);

		vasc_error!("{}: {}", "Message".bold(), message);
		...
		if let Some(location) = panic_info.location() {
			...
			vasc_error!("{}: {}: {}", "Location".bold(), location.file(), location.line());
		} else {
		...
		if verbosity != LevelFilter::Trace {
			report.push_str("Verbose output disabled, run vasc with `-vvvv` flag to enable");

			vasc_error!(
				"{}: Run vasc with {} flag to show detailed tracing",
				"Verbosity".bold(),
				"-vvvv".bold()
			);
		} else {
		...
		if backtrace_enabled {
			...
			report.push_str("Backtrace disabled, run vasc with `--backtrace` flag to enable");

			vasc_error!(
				"{}: Run vasc with {} flag to show full backtrace\n",
				"Backtrace".bold(),
				"--backtrace".bold()
			);
		}
		...
		if report_issue {
			...
			url.push_str(&format!("/issues/new?title=vasc crashed: {message}&body="));
